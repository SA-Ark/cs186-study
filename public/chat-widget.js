...
(function () {
  ...
  const SYSTEM_PROMPT = `You are a CS 186 (Database Systems) study assistant for UC Berkeley Spring 2026 Midterm 1.
...
If asked about topics not on this midterm, say so and redirect to relevant topics.`;
  ...
  const LS_KEY = 'cs186_anthropic_key';
  ...
  // Build chat toggle button
  const toggleBtn = el('button', { id: 'chat-toggle', title: 'Ask Claude' });
  ...
  // Build chat panel
  const chatPanel = el('div', { id: 'chat-panel' });
  ...
  // Send button
  const sendButton = el('button', { id: 'send-btn', className: 'chat-btn', title: 'Send (Enter)' });
  sendButton.appendChild(svgIcon('M2.01 21L23 12 2.01 3 2 10l15 2-15 2z'));

  // Input area
  const inputArea = el('div', { id: 'chat-input-area' }, [
    voiceButton,
  ...
  // Key setup overlay
  const keySetupDiv = el('div', { id: 'key-setup' }, [
  ...
  toggleBtn.addEventListener('click', function () {
    ...
    if (chatPanel.classList.contains('open')) {
      keySetupDiv.style.display = apiKey ? 'none' : 'flex';
  ...
  function renderAssistantText(div, text) {
    ...
    parts.forEach(function (part) {
      if (part.startsWith('```')) {
        ...
        // Process inline formatting with safe methods
